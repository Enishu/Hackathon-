...
export const getCategoryById = async (req, res) => {
    try {
        ...
        res.status(500).json({
            error: "Erreur serveur lors de la recuperation de la categorie",
        });
    }
};
    ...
    try {
        ...
        if (!name || !iconUrl) {
            return res.status(400).json({
                error: "Le nom et l'URL de l'icône sont requis",
            });
        }
        ...
        res.status(500).json({
            error: "Erreur serveur lors de la création de la categorie",
        });
    }
};
