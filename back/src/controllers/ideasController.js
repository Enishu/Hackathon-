...
export const getAllIdeas = async (req, res) => {
    try {
        ...
        const enrichedIdeas = await Promise.all(
            ideas.map(async (idea) => {
                ...
                try {
                    ...
                    console.warn(
                        `Erreur lors de la récupération de l'auteur pour l'idée ${idea.id}:`,
                        error
                    );
                }
                ...
                try {
                    ...
                    console.warn(
                        `Erreur lors du comptage des likes pour l'idée ${idea.id}:`,
                        error
                    );
                }
                ...
                try {
                    ...
                    console.warn(
                        `Erreur lors du comptage des commentaires pour l'idée ${idea.id}:`,
                        error
                    );
                }
        ...
        res.status(500).json({
            error: "Erreur serveur lors de la recuperation des idees",
        });
    }
};
...
export const getIdeaById = async (req, res) => {
    try {
        ...
        res.status(500).json({
            error: "Erreur serveur lors de la recuperation de l'idee",
        });
    }
};
...
export const createIdeas = async (req, res) => {
    try {
        ...
        if (categories && !Array.isArray(categories)) {
            return res.status(400).json({
                error: "Les catégories doivent être un tableau de chaînes",
            });
        }
        ...
        res.status(500).json({
            error: "Erreur serveur lors de la creation de l'idee",
        });
    }
};
...
export const updateIdea = async (req, res) => {
    try {
        ...
        if (result.affectedRows === 0) {
            return res.status(404).json({
                error: "Idee non trouvee ou vous n'etes pas autorise",
            });
        }
        ...
        res.status(500).json({
            error: "Erreur serveur lors de la modification de l'idee",
        });
    }
};
...
export const deleteIdea = async (req, res) => {
    try {
        ...
        res.status(500).json({
            error: "Erreur serveur lors de la suppression de l'idee",
        });
    }
};
...
export const getIdeasByUser = async (req, res) => {
    try {
        ...
        console.error(
            "Erreur lors de la recuperation des idees utilisateur:",
            error
        );
        res.status(500).json({
            error: "Erreur serveur lors de la recuperation des idees utilisateur",
        });
    }
};
